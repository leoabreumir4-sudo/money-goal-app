...
import { getBalanceStatement, getProfiles } from "./wise";
...
export async function handleWiseWebhook(
  ...
  if (!settings?.wiseWebhookSecret) {
    throw new Error("Wise webhook secret not configured");
  }
  ...
  switch (event.event_type) {
    ...
    case "transfers#state-change":
      await handleTransferStateChange(userId, event, settings);
      break;
